<template>
    <b-modal v-model="dialog.show.supplierEvaluation.edit" title="Edit Supplier Evaluation" size="lg">
        ...
        <b-row>
            ...
            <b-col md="6">
                <b-form-group label="Performed by:" label-size="md">
                    <InfiniteScrollSelect v-model="form.performedBy" @onSearch="searchUsers"
                        :selectValue="dialog.temp.supplierEvaluation.data.performed_by_name ? dialog.temp.supplierEvaluation.data.performed_by_name : ''"
                        :options="formattedUsers" :has-next-page="isNextPageAvailable" :loading="busy"
                        @onFetchOptions="searchUsers" @onPaginate="currentPage++" />
                </b-form-group>
            </b-col>
        </b-row>
        ...
        <template #modal-footer>
            <div class="d-flex align-items-center">
                ...
                <b-button class="cursor-pointer" variant="outline-primary" size="sm"
                    @click="dialog.toggleDialog('supplierEvaluation', 'edit')">
...
export default {
    ...
    setup() {
        const { dialog, categories, form: customerSupplierForm } = useCustomerSupplier();
        ...
        const submitForm = () => {
            ...
            customerSupplierForm.supplier_evaluation[dialog.temp.supplierEvaluation.id].evaluation_criteria = data.evaluation_criteria
            customerSupplierForm.supplier_evaluation[dialog.temp.supplierEvaluation.id].evaluation_date = data.evaluation_date
            customerSupplierForm.supplier_evaluation[dialog.temp.supplierEvaluation.id].performed_by = data.performed_by
            customerSupplierForm.supplier_evaluation[dialog.temp.supplierEvaluation.id].performed_by_name = data.performed_by_name
            customerSupplierForm.supplier_evaluation[dialog.temp.supplierEvaluation.id].status = data.status
            customerSupplierForm.supplier_evaluation[dialog.temp.supplierEvaluation.id].total_score = data.total_score

            dialog.temp.supplierEvaluation.data = null;
            dialog.temp.supplierEvaluation.id = null;
        ...
        onMounted(() => {
            fetchUsers();
            form.value.scores = dialog.temp.supplierEvaluation.data.evaluation_criteria;
            form.value.date = dialog.temp.supplierEvaluation.data.evaluation_date;
            form.value.performedBy = dialog.temp.supplierEvaluation.data.performed_by;
        });

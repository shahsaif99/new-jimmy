<template>
    <section class="submitted-checklists">
        <h1 class="mb-1">Submitted checklists</h1>
        ...
        <div class="card">
            ...
            <b-overlay :show="loading" rounded="sm" variant="transparent">
                <b-table
                    ...
                    responsive
                    show-empty
                    empty-text="No submitted checklists"
                    primary-key="id"
                ...
                >
                    ...
                    <template #cell(status)="data">
                        <span
                            class="status-dot"
                            :style="{ background: statusColor(data.item.status) }"
                        ></span>
                    ...
                    <template #cell(progress)="data">
                        <div class="d-flex align-items-center" style="gap: 10px; min-width: 140px">
                            <div class="progress flex-grow-1" style="height: 6px; min-width: 60px">
                                <div
                                    class="progress-bar"
                                    :style="{ width: data.item.progress.percent + '%', background: '#1a90ff' }"
                                ></div>
                            </div>
                    ...
                    <template #cell(deviation_count)="data">
                        <b-badge v-if="data.item.deviation_count > 0" variant="danger" pill>
                            {{ data.item.deviation_count }}
                        </b-badge>
                        <span v-else class="text-muted">—</span>
                    </template>
                    <template #cell(project)="data">
                        <span v-if="data.item.project">{{ data.item.project.project_no ? data.item.project.project_no + ' ' : '' }}{{ data.item.project.name }}</span>
                        <span v-else class="text-muted">—</span>
                    </template>
...
import { BTable, BFormSelect, BPagination, BOverlay, BBadge, BButton, BDropdown, BDropdownItem } from "bootstrap-vue";
    ...
    setup(_, { root }) {
        ...
        async function fetchTemplates() {
            try {
                ...
                if (res.status === 200) {
                    templates.value = res.data.map((c) => ({ id: c.id, name: c.name }));
                }
            } catch (e) {
                templates.value = [];
            }
        ...
        async function confirmDelete(item) {
            const ok = await root.$bvModal.msgBoxConfirm(
                `Delete submission ${item.code}?`,
                { okVariant: "danger", okTitle: "Delete", cancelTitle: "Cancel", centered: true }
            );
            if (!ok) return;
            try {
                await axios.delete(route("submitted-checklists.destroy", item.id));
